...
public class ExcelUtils {
    ...
    // note: to get rows that contain testcaseId
    ...
    public static List<String[]> getNotes(String testCaseId) {
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            if (!currentId.equalsIgnoreCase(testCaseId)) continue;
    ...
    //note: sep. method for negative testcases
    public static String getCellData(String sheetName, String testCaseId, String columnName) {
        try {
            Sheet sheet = workbook.getSheet(sheetName);
            Row headerRow = sheet.getRow(0);
            int columnIndex = -1;
            //note: first find the col. here to match
            ...
            // note: finding matching testcaseid
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                Row row = sheet.getRow(i);
                if (row == null) {
                    continue;
                }
                String currentTcId = row.getCell(0).getStringCellValue();
            ...
            throw new RuntimeException("Failed reading Excel data: " + e.getMessage());
        }
        return "";
    }
}